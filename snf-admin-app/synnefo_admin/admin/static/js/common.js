snf = {
	modals: {
		...
		html: {
			...
			notifySuccess: '<p class="log"><span class="success state-icon snf-font-admin snf-ok"></span>Action <em>"<%= actionName %>"</em><% if (itemsCount>0) { %> for <%= itemsCount %> <% } %> <em class="succeed">succeeded</em>.<%= removeBtn %></p>',
			notifyError: '<div class="log"><%= logInfo %></div>',
			notifyErrorSum: '<p><span class="error state-icon snf-font-admin snf-remove"></span>Action <em>"<%= actionName %>"</em><% if (itemsCount>0) { %> for <%= itemsCount %> <% } %> <em class="error">failed</em>.<%= removeBtn %></p>',
