snf = {
	modals: {
		...
		html: {
			...
			notifyPending: '<p class="log" id="<%= logID %>"><span class="pending state-icon snf-font-admin snf-exclamation-sign"></span>Action <em>"<%= actionName %>"</em><% if (itemsCount>0) { %> for <%= itemsCount %> <% } %> is <em class="pending">pending</em>.<%= removeBtn %></p>',
			notifySuccess: '<p class="log"><span class="success state-icon snf-font-admin snf-ok"></span>Action <em>"<%= actionName %>"</em><% if (itemsCount>0) { %> for <%= itemsCount %> <% } %> has <em class="succeed">succeeded</em>.<%= removeBtn %></p>',
			notifyError: '<div class="log"><%= logInfo %></div>',
			notifyErrorSum: '<p><span class="error state-icon snf-font-admin snf-remove"></span>Action <em>"<%= actionName %>"</em><% if (itemsCount>0) { %> for <%= itemsCount %> <% } %> has <em class="error">failed</em>.<%= removeBtn %></p>',
