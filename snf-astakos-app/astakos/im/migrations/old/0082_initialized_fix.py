...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        buggy = orm.ProjectMembership.objects.filter(id__in=ms).\
            exclude(initialized=True)
        print ('Found %s memberships with misconfigured state; fixing... ' %
               len(buggy))
        orm.ProjectMembership.objects.filter(id__in=ms).\
            exclude(initialized=True).update(initialized=True)
